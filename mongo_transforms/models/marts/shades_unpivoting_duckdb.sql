...
WITH unpivoted_data AS (
    ...
    LEFT JOIN (
        ...
        UNION ALL
        SELECT _dlt_parent_id, _dlt_list_idx, CONCAT('shades_', _dlt_list_idx, '_status') AS "Shades_Key", "status" AS "Shades_Value" FROM {{ source('mongo_raw', 'franchise_c_bs__shades') }}
        UNION ALL
...
SELECT
    CONCAT('shades_', _dlt_list_idx) AS "shade_pkey",
    _id,
    description,
    division,
    status,
